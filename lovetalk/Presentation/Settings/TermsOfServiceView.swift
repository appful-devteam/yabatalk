            ...
            ZStack {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 24) {
                        ...
                        termsSection(
                            title: String(localized: "第3条の2（ユーザー投稿コンテンツと迷惑行為への対応）", bundle: LanguageManager.appBundle),
                            content: String(localized: "1. 本アプリの掲示板・相談部屋等（以下「投稿機能」）では、ユーザーが匿名で体験談や相談を投稿・共有できます。\n2. 運営者は、不適切なコンテンツおよび迷惑・悪質な行為を一切許容しません（ゼロ・トレランス）。次の内容を含む投稿は固く禁止します：\n   • 誹謗中傷、侮辱、嫌がらせ、いじめ、差別的表現\n   • わいせつ・性的に露骨な内容、児童に対する性的搾取に関する内容\n   • 暴力的・脅迫的な内容、自傷・自殺を誘発する内容\n   • 他者の個人情報の無断公開（晒し行為）\n   • スパム、宣伝、なりすまし、その他運営者が不適切と判断する内容\n3. ユーザーは、投稿機能の利用にあたり、上記禁止内容を投稿しないことに同意するものとします。本規約に同意できない場合、投稿機能を利用することはできません。\n4. 本アプリは、投稿時の不適切表現フィルタ、投稿の通報機能、ユーザーのブロック機能、および自分の投稿をいつでも削除できる機能を備えています。\n5. 運営者は、通報を受けたコンテンツを24時間以内に確認し、本規約に違反すると判断した場合は当該コンテンツを削除し、違反したユーザーのアカウント利用を停止（排除）します。\n6. 不適切な投稿・迷惑行為の報告は、アプリ内の通報機能、または「設定 > お問い合わせ」（info@appful.tokyo）から行うことができます。", bundle: LanguageManager.appBundle)
                        )

                        termsSection(
                            title: String(localized: "第4条（トーク履歴データの取り扱い）", bundle: LanguageManager.appBundle),
                            content: String(localized: "1. ユーザーがインポートしたトーク履歴データは、原則としてユーザーのデバイス内にのみ保存されます。\n2. AIサマリー機能、返信提案（相談）機能、または擬人化チャット機能を利用する場合、トーク履歴の一部（メッセージ本文、送信者名、送信日時）がAlibaba Cloud Singapore Pte. Ltd.（Qwen API）のサーバー（シンガポール）に送信されます。画像・動画・スタンプ等のメディアデータは送信されません。\n3. ユーザーは、これらのAI機能の初回利用時に、データの外部送信に関する明示的な同意を行う必要があります。各機能の初回利用時に、送信されるデータの内容、送信先（Alibaba Cloud Singapore Pte. Ltd.）、およびAlibaba Cloud によるデータの取り扱いについて明記した同意画面が表示されます。\n4. Alibaba Cloud によるデータの取り扱いについては、Alibaba Cloud のプライバシーポリシー（https://www.alibabacloud.com/help/en/legal/latest/alibaba-cloud-international-website-privacy-policy）およびQwen API利用規約（https://qwen.ai/termsservice）が適用されます。\n5. AI機能を利用しない場合、トーク履歴がデバイス外に送信されることはありません。", bundle: LanguageManager.appBundle)
                        )

                        termsSection(
                            title: String(localized: "第5条（トーク相手の個人情報に関するユーザーの責任）", bundle: LanguageManager.appBundle),
                            content: String(localized: "1. トーク履歴にはユーザー以外の方（トーク相手）の発言、氏名、その他の個人情報が含まれています。\n2. 日本の個人情報保護法に基づき、第三者の個人情報を外部サービス（Qwen API を含む）に提供する際は、原則としてその方の同意が必要です。AI機能（AIサマリー・返信提案・擬人化チャット）を利用する場合、トーク相手の個人情報がAlibaba Cloud Singapore Pte. Ltd. のサーバーに送信されることをご理解ください。\n3. ユーザーは、本アプリの利用にあたり、トーク相手のプライバシーに十分配慮し、必要に応じてトーク相手の同意を得る等、法令を遵守するための適切な措置を講じる責任を負います。\n4. LINEの利用規約では、第三者の個人情報を不正に収集・開示・提供する行為が禁止されています。ユーザーは、LINEの利用規約を遵守した上で本アプリを利用してください。\n5. トーク相手のプライバシーに関して生じた紛争、損害、請求等について、運営者は一切の責任を負いません。", bundle: LanguageManager.appBundle)
                        )
