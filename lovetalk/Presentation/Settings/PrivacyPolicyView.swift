                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 24) {
                        ...
                        policySection(
                            ...
                            content: String(localized: "本アプリは、以下の場合を除き、ユーザーの個人情報を第三者に提供することはありません：\n\n(1) ユーザーが明示的に同意した場合\nAIサマリー機能、返信提案（相談）機能、または擬人化チャット機能の初回利用時に表示される同意画面にて、データ送信に同意した場合。\n\n(2) 法令に基づく場合\n法律の規定に基づき、裁判所、警察その他の行政機関から開示を求められた場合。\n\n本アプリが利用する外部サービス：\n• Apple App Store（アプリ配信およびサブスクリプション決済）\n• Qwen API（AIサマリー・返信提案・擬人化チャット） — 運営: Alibaba Cloud Singapore Pte. Ltd.\n• Firebase（Firebase Analytics・Crashlytics・Cloud Functions・Cloud Firestore・Authentication・Remote Config・Cloud Messaging） — 運営: Google LLC\n• Google AdMob（広告配信） — 運営: Google LLC", bundle: LanguageManager.appBundle)
                        )

                        policySection(
                            title: String(localized: "5. AI（生成AI）の利用について", bundle: LanguageManager.appBundle),
                            content: String(localized: "AIサマリー機能、返信提案（相談）機能、および擬人化チャット機能では、Alibaba Cloud Singapore Pte. Ltd. が提供する生成AI「Qwen API」を利用しています。\n\n(1) 送信先\nAlibaba Cloud Singapore Pte. Ltd.（Qwen API）\n所在地・処理地域: シンガポール\n\n(2) 送信されるデータ\nトーク履歴のメッセージ本文、送信者名、送信日時（画像・動画・スタンプ等のメディアデータは送信されません）。\n\n(3) データの取り扱い\n送信されたデータの取り扱いは、Alibaba Cloud の利用規約およびプライバシーポリシーに従います。詳細は下記のリンクをご参照ください。\n\n(4) 同意の取得\n本機能はユーザーの明示的な同意を得た場合のみ有効になります。各AI機能の初回利用時に、送信されるデータの内容・送信先・データの取り扱いを明記した同意画面が表示され、ユーザーが明示的に同意した場合のみデータが送信されます。\n\n(5) 詳細\nAlibaba Cloud プライバシーポリシー: https://www.alibabacloud.com/help/en/legal/latest/alibaba-cloud-international-website-privacy-policy\nQwen API 利用規約: https://qwen.ai/termsservice", bundle: LanguageManager.appBundle)
                        )
